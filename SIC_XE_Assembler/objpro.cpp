...
void obpro() {
	...
	writefile.open("..\\object_program.txt");
	getline(myfile, line);
	string temp[5] = {};
	...
	{	
		istringstream data(line);
		// 0:LOC, 1:symbol, 2:opc, 3:ins, 4:OBC
		data >> temp[0] >> temp[1] >> temp[2] >> temp[3] >> temp[4];
		if (temp[4] == "NULL") {
			if (temp[2] == "RESW" || temp[2] == "RESB") {
				if (Tcount != "") {
					cout << "T^" << setfill('0') << setw(6) << first_loc.substr(2) << "^" << setfill('0') << setw(2) << count << Tcount << endl;
					writefile << "T^" << setfill('0') << setw(6) << first_loc.substr(2) << "^" << setfill('0') << setw(2) << count << Tcount << "\n";
					count = 0;
					Tcount = "";
				}
			}
			...
			if (count + (int)temp[4].length() / 2 > 30) {
				cout << "T^" << setfill('0') << setw(6) << first_loc.substr(2) << "^" << setfill('0') << setw(2) << count << Tcount << endl;
				writefile << "T^" << setfill('0') << setw(6) << first_loc.substr(2) << "^" << setfill('0') << setw(2) << count << Tcount << "\n";
